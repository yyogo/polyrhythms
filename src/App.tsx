...
const BeatVisualizer = React.memo(
  ({
    beats,
    currentBeat,
    color,
    ...
    >
      ...
      <CardContent>
        <section>
          ...
          <p>
            A <b>polyrhythm</b> happens when two or more different rhythms are
            played at the same time. For example, if one hand plays 3 beats
            while the other plays 2 beats in the same time span, that's a 3:2
            polyrhythm. It creates interesting patterns that might sound complex
            at first, but they occur naturally in many styles of music around
            the world.
          </p>
          <p>
            This app helps you practice or play around with polyrhythms by
            visualizing and playing multiple rhythms simultaneously.
          </p>
        </section>
        <section>
          <h3>Instructions</h3>
          <p>
            Select the beat count (1-11) for each rhythm using the sliders. Set
            a slider to 0 to disable that rhythm.
          </p>
          <p>
            Adjust tempo using the BPM slider or tap the tempo button
            repeatedly.
          </p>
          <p>
            Press the play button to start the rhythms. The visualizer will show
            the current beat for each rhythm.
          </p>
        </section>
          ...
          <table>
            <tbody>
              ...
              <tr>
                ...
                <td>Focus rhythm sliders</td>
              </tr>
              <tr>
                <td>
                  <kbd>Q</kbd>/<kbd>E</kbd>/<kbd>W</kbd>
                </td>
                <td>
                  Increase/decrease/tap tempo (hold{" "}
                  <kbd data-key="Shift">Shift</kbd> for smaller increment)
                </td>
              </tr>
              <tr>
                <td>
      ...
      <CardActions>
        <Button
          size="small"
          href="https://github.com/yyogo/polyrhythms"
          target="_blank"
        >
...
function buildGainEnvelope(
  ...
  source.start(start);
  source.stop(
    start + envelope.attack + envelope.decay + envelope.release + 0.01
  );
  source.onended = () => {
    source.disconnect();
    gainNode.disconnect();
...
const PolyrhythmPlayground = () => {
  ...
  const [currentBeats, setCurrentBeats] = useState<Array<number | null>>([
    null,
    null,
    null,
    null,
  ...
  const measureCursorRefs = useRef<(HTMLDivElement | null)[]>(
    Array.from({ length: 5 }).map(() => null)
  );
  const setMeasureCursor = useCallback(
  ...
  const handleTap = () => {
    ...
    if (
      timestamps.length > 0 &&
      now - timestamps[timestamps.length - 1] > 2000
    ) {
      timestamps.length = 0;
    }
  ...
  const playBeat = (index: number) => {
    ...
    buildGainEnvelope(audioContextRef.current, oscillator, envelope).connect(
  ...
  const frameHandler = useCallback(
    (timestamp: DOMHighResTimeStamp) => {
      ...
      const newBeats = rhythms.map((rhythm, index) => {
        ...
        if (
          rhythm &&
          (currentBeat !== currentBeatsRef.current[index] ||
      ...
      measurePosRef.current = newMeasurePos;
      measureCursorRefs.current.forEach((ref) =>
        ref?.style.setProperty("left", `${newMeasurePos * 100}%`)
      );
  ...
  const updateRhythm = (id: number, beats: number) => {
    setRhythms((prev) =>
      prev.map((orig, index) => (id === index ? beats : orig))
    );
  };
          ...
          <Stack spacing={4}>
            ...
            >
              <IconButton
                ...
                sx={{
                  display: { xs: "none", sm: "flex", border: "1px solid " },
                }}
            ...
            >
              ...
              <Stack
                spacing={1}
                sx={{ flex: 4, justifyContent: "space-between" }}
              >
                {Array.from({ length: 5 }).map((_, index) => (
                  <Box key={index} sx={{ position: "relative" }}>
                    {isPlaying && (
                      <MeasureCursor
                        ref={(el) => setMeasureCursor(el, index)}
                      />
                    )}
                    <BeatVisualizer
                      beats={rhythms[index]}
